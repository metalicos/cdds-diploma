...
import ua.com.cyberdone.devicemicroservice.device.hydroponic.v1.repos.SpecialSystemRepository;
...
public class SpecialSystemService implements ModelEntityMapper<SpecialSystem, SpecialSystemDTO> {
    ...
    public SpecialSystemDTO create(final SpecialSystemDTO specialSystemDTO) {
        final SpecialSystem specialSystem = new SpecialSystem();
        mapToEntity(specialSystemDTO, specialSystem);
        return mapToDTO(specialSystemRepository.save(specialSystem), new SpecialSystemDTO());
    }

    public SpecialSystemDTO update(final Long id, final SpecialSystemDTO specialSystemDTO) {
    ...
    public SpecialSystemDTO findByDeviceUuid(String uuid) {
        return specialSystemRepository.findByDeviceUuid(uuid)
                .map(specialSystem -> mapToDTO(specialSystem, new SpecialSystemDTO()))
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND));
    }
}
